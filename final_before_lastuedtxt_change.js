...
function innerText (id,lastUsedTxt){
    let txtInput = document.getElementById(id);
    let currentEle = txtInput.id.startsWith('cta_') ? document.getElementById(lastUsedTxt).querySelector('.cta') : document.getElementById(lastUsedTxt);
    currentEle.innerHTML = txtInput.value 
    // updateObj();
}

function cssChange (id,lastUsedTxt,getCssProp,row_count,section){
    let txtInput = document.getElementById(id);
    let currentEle = txtInput.id.startsWith('cta_') ? document.getElementById(lastUsedTxt).querySelector('.cta') : document.getElementById(lastUsedTxt);
    currentEle.style[getCssProp] = txtInput.value + "px"

    
    updateObj(id,lastUsedTxt,getCssProp,row_count,section);
}

function textTransform(id,lastUsedTxt,getCssProp,row_count,section){
    let txtInput = document.getElementById(id);
    let currentEle = txtInput.id.startsWith('cta_') ? document.getElementById(lastUsedTxt).querySelector('.cta')  : document.getElementById(lastUsedTxt) ;
...
function textItalic(id,lastUsedTxt,getCssProp,row_count,section){
    let txtInput = document.getElementById(id);
    let currentEle = txtInput.id.startsWith('cta_') ? document.getElementById(lastUsedTxt).querySelector('.cta')  : document.getElementById(lastUsedTxt) ;
...
function color_bg_change(id,lastUsedTxt,getCssProp,hex_inpput,row_count,section){
    let txtInput = document.getElementById(id);
    ...
    if(txtInput.id.startsWith('cta')){
        let currentEle = document.getElementById(lastUsedTxt).querySelector('.cta')
        currentEle.style[getCssProp] = color;
        console.log('.cta');
    }else if (txtInput.id.startsWith('bgCta')){
        let currentEle = document.getElementById(lastUsedTxt).querySelector('.ctaWrap')
        currentEle.style[getCssProp] = color;
        console.log('.ctaWrap');
    }else{  
...
function gredientBg (id,lastUsedTxt,getCssProp,row_count,section){
    let txtInput = document.getElementById(id);
    let currentEle = txtInput.id.startsWith('cta') ? document.getElementById(lastUsed_cta).querySelector('.ctaWrap') : document.getElementById(lastUsedTxt);
...
// background image 
function bgImage(id,lastUsedTxt,getCssProp,row_count,section){
    let txtInput = document.getElementById(id);
...
function bgPos (id,lastUsedTxt,getCssProp,posId,row_count,section){
    let txtInput = document.getElementById(id);
    let currentEle = txtInput.id.startsWith('cta_') ?  document.getElementById(lastUsedTxt).querySelector('.cta') :  document.getElementById(lastUsedTxt) ;
...
function bgPosUnit (lastUsedTxt,getCssProp,posId,row_count,section){
    ...
    updateObj(lastUsedTxt,getCssProp,row_count,section);

}
// background image  end

function imgSrc(id,lastUsedTxt){
    let txtInput = document.getElementById(id);
    let [file] = txtInput.files
    if (file) {
        let currentEle = txtInput.id.startsWith('cta') ? document.getElementById(lastUsedTxt).querySelector('.arrowImage') :  document.getElementById(lastUsedTxt);
        currentEle.src  =  URL.createObjectURL(file) 
    };
    // updateObj();
}

function imgAttr (id,lastUsedTxt,getCssProp,row_count,section){
    let txtInput = document.getElementById(id);
    let currentEle = txtInput.id.startsWith('cta_') ? document.getElementById(lastUsedTxt).querySelector('.arrowImage') : document.getElementById(lastUsedTxt);
...
function toggleView (id,txtInputVal,lastUsedTxt, getCssProp,section){
        ...
        if(id == 'mobView'){
            contWrap.forEach(cls => {
                ...
                if(!txtInputVal.files) {
                    txtInputVal.value = !config.mobile[section][lastUsedTxt] ? config.desktop[section][lastUsedTxt][getCssProp] :  config.mobile[section][lastUsedTxt][getCssProp];
                }
            ...
            contWrap.forEach(cls => {
                ...
                if(!txtInputVal.files) {
                    txtInputVal.value = config.desktop[section][lastUsedTxt][getCssProp];
                }
    ...
    function updateObj(id,lastUsedTxt,getCssProp,row_count,section) { 
        ...
        for(i = 1; i <= config[row_count]; i++){
            ...
            if (mobWidth == '320px'){
                    if(!config.mobile[section][lastUsedTxt]) config.mobile[section][lastUsedTxt] = {}
                    config.mobile[section][lastUsedTxt][getCssProp] = txtInput.value ;
            } else{
                    if(!config.desktop[section][lastUsedTxt]) config.desktop[section][lastUsedTxt] = {}
                    config.desktop[section][lastUsedTxt][getCssProp] = txtInput.value;
...
function get_mHead_id(clicked_id_txt){
    ...
    function updatecssValue (id, getCssProp){
        ...
        mob_View.addEventListener("click", function() {toggleView('mobView', txtInput, lastUsedTxt, getCssProp,'text')});
        let desk_view = document.getElementById("deskView")
        desk_view.addEventListener("click", function() {toggleView('deskView', txtInput, lastUsedTxt, getCssProp,'text')});
        
        
        let txtInput = document.getElementById(id);
        txtInput.addEventListener('input', (e) => {
            if(e.currentTarget.id == "fSize1") cssChange('fSize1',lastUsedTxt, getCssProp,'total_text_Row','text');
            if(e.currentTarget.id == "lHeight") cssChange('lHeight',lastUsedTxt, getCssProp,'total_text_Row','text');
            if(e.currentTarget.id == "lSpacing") cssChange('lSpacing',lastUsedTxt, getCssProp,'total_text_Row','text');
            if(e.currentTarget.id == "paddTop") cssChange('paddTop',lastUsedTxt, getCssProp,'total_text_Row','text');
            if(e.currentTarget.id == "paddBtm") cssChange('paddBtm',lastUsedTxt, getCssProp,'total_text_Row','text');
            if(e.currentTarget.id == "paddLeft") cssChange('paddLeft',lastUsedTxt, getCssProp,'total_text_Row','text');
            if(e.currentTarget.id == "paddRight") cssChange('paddRight',lastUsedTxt, getCssProp,'total_text_Row','text');
            if(e.currentTarget.id == "uCase") textTransform('uCase',lastUsedTxt,'textTransform','total_text_Row','text');
            if(e.currentTarget.id == "txtBg") bgImage('txtBg',lastUsedTxt, getCssProp,'total_text_Row','text');
            if(e.currentTarget.id == "bg_pos_x")bgPos ('bg_pos_x',lastUsedTxt, getCssProp,'pos_x_drop','total_text_Row','text');
            if(e.currentTarget.id == "bg_pos_y")bgPos ('bg_pos_y',lastUsedTxt, getCssProp,'pos_y_drop','total_text_Row','text');
            if(e.currentTarget.id == "tItalic") textItalic('tItalic',lastUsedTxt, getCssProp,'total_text_Row','text');
            if(e.currentTarget.id == "color") color_bg_change('color',lastUsedTxt, getCssProp,'hex','total_text_Row','text');
            if(e.currentTarget.id == "txtInput") innerText('txtInput',lastUsedTxt);
        });
        ...
        if(id == "bg_pos_x") {txtInput.value = parseInt(getStyle_txt.backgroundPositionX);
            bgPosUnit (lastUsedTxt,getCssProp,'pos_x_drop');
        };
        if(id == "bg_pos_y") {txtInput.value = parseInt(getStyle_txt.backgroundPositionY);
            bgPosUnit (lastUsedTxt,getCssProp,'pos_y_drop');
        }
...
function get_cta_id(clicked_id_cta) {
    ...
    let txt_id = document.getElementById(lastUsed_cta).querySelector('.cta');
    let getStyle_txt = getComputedStyle(txt_id); 
    ...
    function updateCtaCssValue (id, getCssProp){
        ...
        txtInput.addEventListener('input', (e) => {
            if(e.currentTarget.id == "cta_Size") cssChange('cta_Size',lastUsed_cta, getCssProp, 'total_cta_Row','cta');
            if(e.currentTarget.id == "cta_lHeight") cssChange('cta_lHeight',lastUsed_cta, getCssProp,'total_cta_Row','cta');
            if(e.currentTarget.id == "ctacolor") color_bg_change('ctacolor',lastUsed_cta, getCssProp,'ctahex');
            if(e.currentTarget.id == "bgCta") color_bg_change('bgCta',lastUsed_cta, getCssProp,'ctabghex');
            if(e.currentTarget.id == "cta_grad") gredientBg('cta_grad',lastUsed_cta, );
            if(e.currentTarget.id == "cta_txt") innerText('cta_txt',lastUsed_cta);
            if(e.currentTarget.id == "cta_arrImg") imgSrc('cta_arrImg',lastUsed_cta);
